...
const AdminMealPlansPage: React.FC = () => {
  ...
  const fetchMealPlans = async () => {
    try {
      ...
      // For admin, we'll use a different endpoint or modify the existing one
      const response = await api.getAllMealPlans(params);
      if (response.success) {
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600">
                Manage and monitor all meal plans in the system
              </p>
            </div>
            <div className="flex items-center space-x-3">
              <Link to="/admin/meal-plans/create">
          ...
          <div>
            ...
            {mealPlans.length > 0 ? (
                    ...
                    {mealPlans.map((mealPlan) => (
                      <tr key={mealPlan.id} className="hover:bg-gray-50">
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div>
                            <div className="text-sm font-medium text-gray-900">{mealPlan.name}</div>
                            <div className="text-sm text-gray-500">{mealPlan.description}</div>
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm text-gray-900">
                            {mealPlan.user ? `${mealPlan.user.first_name} ${mealPlan.user.last_name}` : `User ${mealPlan.user_id}`}
                          </div>
                          <div className="text-sm text-gray-500">
                            {mealPlan.user?.email || 'No email'}
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                          {getDuration(mealPlan.start_date, mealPlan.end_date)} days
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                            ...
                            {mealPlan.is_ai_generated ? 'AI Generated' : 'Manual'}
                                                 ...
                                                 <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                           {typeof mealPlan.total_calories === 'number' ? mealPlan.total_calories.toFixed(0) : 'N/A'} cal
                         </td>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                          <div className="flex items-center space-x-2">
                            ...
                            <Link to={`/admin/meal-plans/${mealPlan.id}/edit`}>
              ...
              <div className="text-center py-8">
                ...
                <p className="text-gray-600">No meal plans match your current filters.</p>
              </div>
            )}
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 max-w-6xl w-full mx-4 max-h-[90vh] overflow-y-auto">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                <div>
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <div><strong>User:</strong> {selectedMealPlan.user ? `${selectedMealPlan.user.first_name} ${selectedMealPlan.user.last_name}` : `User ${selectedMealPlan.user_id}`}</div>
                ...
                <div>
                  <h4 className="font-medium text-gray-900 mb-2">Nutrition Summary</h4>
                                     <div className="space-y-2 text-sm">
                     <div><strong>Total Calories:</strong> {typeof selectedMealPlan.total_calories === 'number' ? selectedMealPlan.total_calories.toFixed(0) : 'N/A'} cal</div>
                     <div><strong>Total Protein:</strong> {typeof selectedMealPlan.total_protein === 'number' ? selectedMealPlan.total_protein.toFixed(1) : 'N/A'}g</div>
                     <div><strong>Total Carbs:</strong> {typeof selectedMealPlan.total_carbs === 'number' ? selectedMealPlan.total_carbs.toFixed(1) : 'N/A'}g</div>
                     <div><strong>Total Fat:</strong> {typeof selectedMealPlan.total_fat === 'number' ? selectedMealPlan.total_fat.toFixed(1) : 'N/A'}g</div>
                     <div><strong>Meals Planned:</strong> {selectedMealPlan.meals?.length || 0}</div>
                   </div>
                </div>
              </div>
              ...
              <div className="flex justify-end space-x-2">
                ...
                <Link to={`/admin/meal-plans/${selectedMealPlan.id}/edit`}>
                  <Button variant="primary">
                    Edit Meal Plan
                  </Button>
                </Link>
